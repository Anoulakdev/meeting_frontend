...
import { MeetingDocForm, emptyDocForm } from "./AddModal";
...
export function EditModal({ open, onClose, onSuccess, selectedDoc }: EditModalProps) {
  ...
  const [errors, setErrors] = useState<Partial<MeetingDocForm>>({});
  ...
  const validate = (): boolean => {
    const newErrors: Partial<MeetingDocForm> = {};
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.put(`/api/meetingdocs/${selectedDoc.id}`, data, {
      ...
      console.error("[EditModal] PUT /api/meetingdocs failed:", err);
      const msg =
  ...
  return (
    <Modal open={open} onClose={onClose} title="ແກ້ໄຂເອກະສານ">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input label="ວັນທີເລີ່ມ *" type="date" value={form.startDate} onChange={set("startDate")} error={errors.startDate} />
          <Input label="ວັນທີສິ້ນສຸດ *" type="date" value={form.endDate} onChange={set("endDate")} error={errors.endDate} />
        </div>

        {/* Times */}
        <div className="grid grid-cols-2 gap-4">
          <Input label="ເວລາເລີ່ມ *" type="time" value={form.startTime} onChange={set("startTime")} error={errors.startTime} />
          <Input label="ເວລາສິ້ນສຸດ *" type="time" value={form.endTime} onChange={set("endTime")} error={errors.endTime} />
        </div>

        {/* Location */}
        <Input label="ສະຖານທີ່ *" value={form.location} onChange={set("location")} error={errors.location} />

        {/* File Upload */}
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium" style={{ color: "rgb(var(--text-primary))" }}>
            ໄຟລ໌ເອກະສານ (PDF) — ອັບໂຫຼດໃໝ່ຖ້າຕ້ອງການປ່ຽນ
          </label>
          ...
          {file ? (
            ...
            >
              ...
              <span className="text-sm flex-1 truncate" style={{ color: "rgb(var(--text-primary))" }}>{file.name}</span>
            ...
            <button
              ...
              style={{ background: "rgb(var(--bg))", border: "1px dashed rgb(var(--border))", color: "rgb(var(--text-secondary))" }}
            >
              <Upload className="w-4 h-4" />
              {selectedDoc?.docfile ? `ໄຟລ໌ປັດຈຸບັນ: ${selectedDoc.docfile} (ຄລິກເພື່ອປ່ຽນ)` : "ເລືອກໄຟລ໌ PDF..."}
            </button>
          )}
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <Button variant="secondary" onClick={onClose} className="flex-1">ຍົກເລີກ</Button>
          <Button variant="primary" onClick={handleSubmit} loading={saving} className="flex-1">ອັບເດດ</Button>
        </div>
      </div>
    </Modal>
  );
