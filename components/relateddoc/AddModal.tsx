...
import { Department } from "@/schemas/relatedDoc";

export interface RelatedDocForm {
  title: string;
  description: string;
  departmentId: string;
}

export const emptyRelatedDocForm: RelatedDocForm = {
  title: "",
  description: "",
  departmentId: "",
};
...
export function AddModal({ open, onClose, onSuccess }: AddModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.post("/api/relateddocs", data, {
      ...
      console.error("[AddModal] POST /api/relateddocs failed:", err);
      const msg =
  ...
  return (
    <Modal open={open} onClose={onClose} title="ເພີ່ມເອກະສານທີ່ຕິດພັນ" size="lg">
        ...
        <div className="space-y-1.5">
          ...
          {!file ? (
            <div
              ...
              className="border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all hover:opacity-80"
              style={{
